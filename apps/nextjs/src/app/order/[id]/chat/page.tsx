...
export default function ChatPage({
  ...
  const { data: messages, isLoading } = useQuery({
    ...trpc.chat.getMessages.queryOptions({ orderId }),
  ...
  const { broadcastChatEvent } = useOrderRealtime(
    paymentConfig?.databaseReady === true ? orderId : "",
    {
      onChatUpdate: () => {
        void queryClient.invalidateQueries({
          queryKey: trpc.chat.getMessages.queryKey({ orderId }),
        });
      },
    },
  );
  ...
  return (
    <div className="flex h-screen flex-col bg-zinc-950">
      <div className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900 p-4 shadow-md">
        ...
        <button
          onClick={() =>
            router.push(isAccepted ? `/` : `/order/${orderId}/tracker`)
          }
      ...
      <div className="flex-1 overflow-y-auto p-4" ref={scrollRef}>
        <div className="flex min-h-full flex-col-reverse justify-end gap-4">
          {messages?.map((msg) => {
            ...
            return (
              <div
                ...
                className={`flex max-w-[80%] flex-col rounded-2xl p-4 ${
                  isMe
                    ? "self-end rounded-br-none bg-purple-600"
                    : "self-start rounded-bl-none bg-zinc-800"
                }`}
              >
                {!isMe && (
