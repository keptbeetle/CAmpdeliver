      ...
      <header className="sticky top-0 z-40 w-full border-b border-zinc-800/80 bg-zinc-950/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-md items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-3">
            {profile && (
              <div className="flex items-center gap-1.5 rounded-full border border-purple-500/30 bg-purple-950/40 px-2.5 py-1 text-xs font-semibold text-purple-300">
                <Wallet className="h-3.5 w-3.5" />
                <span>₹{(profile.walletBalance / 100).toFixed(0)}</span>
              </div>
            )}
            ...
            <div className="flex-1 space-y-1 overflow-y-auto py-2">
              ...
              {profile?.role === "ADMIN" && (
                <div className="mt-4 border-t border-zinc-800/80 pt-4">
                  <p className="mb-2 px-3.5 text-xs font-bold text-amber-400/90 uppercase tracking-wider flex items-center gap-1.5">
